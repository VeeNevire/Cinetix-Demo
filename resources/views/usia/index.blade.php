    ...
    <div class="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-8">
        <div data-aos="fade-up">
            <span class="inline-flex items-center gap-2 bg-cinema-accent/10 text-cinema-accent-light px-3 py-1 rounded-full text-xs font-semibold">
                <i class="bi bi-people"></i> Filter
            </span>
            <h1 class="text-2xl md:text-3xl font-bold text-white mt-2 font-display">Filter Berdasarkan Usia</h1>
        </div>

        <div class="flex flex-wrap gap-2 mt-6" data-aos="fade-up">
            @php $usiaList = ['SU', '13+', '17+', '21+']; @endphp
            @foreach($usiaList as $u)
                <a href="{{ route('usia.index', ['usia' => $u]) }}" class="px-4 py-2 rounded-full text-sm transition {{ $usia === $u ? 'bg-cinema-accent text-white shadow-lg shadow-cinema-accent/30' : 'bg-cinema-700/50 text-gray-300 hover:bg-cinema-600 hover:text-white border border-cinema-600/30' }}">
                    {{ $u }}
                </a>
            @endforeach
            <a href="{{ route('usia.index', ['usia' => '']) }}" class="px-4 py-2 rounded-full text-sm bg-cinema-700/50 text-gray-300 hover:bg-cinema-600 hover:text-white border border-cinema-600/30 transition">Semua</a>
        </div>

        @if($films->isEmpty())
            <div class="text-center py-16" data-aos="fade-up">
                <i class="bi bi-search text-4xl text-gray-600"></i>
                <p class="text-gray-500 mt-3">Tidak ada film dengan usia "{{ $usia }}"</p>
            </div>
        @else
